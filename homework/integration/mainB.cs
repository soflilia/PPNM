...
class MainINTB{
    public static void Main(){
        ...
        double normal_1 = Int.integrate(counts,0,1);

        int ncalls1 = 0;
        Func <double,double> counts1 = z=> {ncalls1++ ; return 1.0/(Math.Sqrt(z));};
        double trans_1 = Int.Clenshaw_Curtis_VT(counts1,0,1);
        ...
        double normal_2 = Int.integrate(counts2,0,1);

        int ncalls3 = 0;
        Func <double,double> counts3 = z=> {ncalls3++ ; return Math.Log(z)/(Math.Sqrt(z));};
        double trans_2 = Int.Clenshaw_Curtis_VT(counts3,0,1);
